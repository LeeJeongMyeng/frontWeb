...
public class A06_PareparedDao {
	...
	// 조회 처리
	/*
	 * public List<Member> getMemberList(Member sch){ List<Member> Mlist = new
	 * ArrayList<Member>(); String sql = "SELECT * FROM MEMBER\r\n" +
	 * "WHERE name LIKE '%'||?||'%'\r\n" + "and authority LIKE '%'||?||'%'"; try {
	 * con = DB.con(); pstmt = con.prepareStatement(sql); pstmt.setString(1,
	 * sch.getName()); pstmt.setString(2, sch.getAuthority()); rs =
	 * pstmt.executeQuery(); // sql를 넣지 않는다.!!(주의) while(rs.next()) { Mlist.add(new
	 * Member( rs.getString("id"), rs.getString("authority"), rs.getString("name"),
	 * rs.getString("pass"), rs.getInt("point") ) ); }
	 * System.out.println("데이터 건수:"+Mlist.size());
	 * 
	 * } catch (SQLException e) { System.out.println("DB에러:"+e.getMessage()); }
	 * catch(Exception e) { System.out.println("일반 에러:"+e.getMessage()); }finally {
	 * DB.close(rs, pstmt, con); }
	 * 
	 * return Mlist; }
	 */
		...
		/*
		 * public boolean login (String id, String pass){ boolean isSuccess=false;
		 * List<Member> Mlist = new ArrayList<Member>(); String sql =
		 * "SELECT * FROM MEMBER\r\n" + "WHERE id=? and pass=?"; try { con = DB.con();
		 * pstmt = con.prepareStatement(sql); pstmt.setString(1, id); pstmt.setString(2,
		 * pass); rs = pstmt.executeQuery(); // sql를 넣지 않는다.!!(주의) isSuccess=rs.next();
		 * //해당 id,pass로 조회될땜만 true 그 외, false
		 * 
		 * System.out.println("데이터 건수:"+Mlist.size());
		 * 
		 * } catch (SQLException e) { System.out.println("DB에러:"+e.getMessage()); }
	 ...
	 public boolean Dept (int deptno, String dname){
		...
		/*
		 * public boolean checkId (String id){ boolean isSuccess=false; List<Member>
		 * Mlist = new ArrayList<Member>(); String sql = "SELECT * FROM MEMBER\r\n" +
		 * "WHERE id=?"; try { con = DB.con(); pstmt = con.prepareStatement(sql);
		 * pstmt.setString(1, id); rs = pstmt.executeQuery(); // sql를 넣지 않는다.!!(주의)
		 * isSuccess=rs.next(); //해당 id,pass로 조회될땜만 true 그 외, false
		 * 
		 * System.out.println("데이터 건수:"+Mlist.size());
		 * 
